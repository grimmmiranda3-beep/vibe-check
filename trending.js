...
(function () {
  ...
  function render() {
    ...
    if (!panel) { panel = document.createElement('section'); panel.id = 'vibelyTrending'; panel.className = 'trending-panel'; host.appendChild(panel); }
    if (!places.length) { panel.innerHTML = '<div class="trending-head"><div><h2>🔥 Trending right now</h2><p>Search a place type and city to see what is standing out.</p></div></div>'; return; }
    panel.innerHTML = `<div class="trending-head"><div><h2>🔥 Trending right now</h2><p>Top vibe scores from your current local search.</p></div><span class="meta">Live search results</span></div><div class="trending-grid">${places.map((p,i)=>`<div class="trend-card"><div class="trend-rank">#${i+1} Trending</div><div class="trend-name">${esc(p.emoji)} ${esc(p.name)}</div><div class="trend-score">${Number(p.score).toFixed(1)}<span class="meta"> / 10</span></div><div class="trend-meta">${esc(p.type)} · ${p.openNow?'Open now':'See place status'}</div><div class="trend-vibes">${(p.tags||[]).slice(0,3).map(t=>`<span class="trend-vibe">${esc(t)}</span>`).join('')}</div></div>`).join('')}</div>`;
  }
