// Vibe Check — vibe-first discovery.
// IMPORTANT: no star ratings, review scores, or Yelp-style ranking language.
(function () {
  'use strict';
  const esc = (v) => String(v ?? '').replace(/[&<>\"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]));
  const LABELS = {'😍':'Loved','😊':'Chill','🔥':'Energetic','😌':'Relaxed','🥳':'Party'};
  let latestPlaces = [];
  let timer = null;
  ...
  function styles(){
    ...
    s.textContent=`.trending-panel{margin:22px 0 0;background:#fff;border:1px solid var(--line);border-radius:22px;padding:20px;box-shadow:0 8px 28px #21142c08}.trending-head{display:flex;justify-content:space-between;gap:12px;align-items:end;margin-bottom:14px}.trending-head h2{margin:0;font-size:21px}.trending-head p{margin:4px 0 0;color:var(--muted);font-size:13px}.trending-live{font-size:11px;font-weight:900;color:var(--accent)}.trending-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.trend-card{border:1px solid var(--line);border-radius:18px;padding:16px;background:linear-gradient(180deg,#fff,#faf8fb);cursor:pointer;transition:.18s}.trend-card:hover{transform:translateY(-2px);box-shadow:0 10px 28px #21142c12}.trend-rank{font-size:11px;font-weight:900;color:var(--accent);text-transform:uppercase;letter-spacing:.06em}.trend-name{font-size:16px;font-weight:900;margin:7px 0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.trend-vibe{font-size:34px;margin:8px 0 2px}.trend-label{font-weight:900}.trend-meta{font-size:12px;color:var(--muted);margin-top:5px;line-height:1.45}.trend-pulse{margin-top:12px;font-size:12px;font-weight:850}.trend-breakdown{display:flex;gap:5px;flex-wrap:wrap;margin-top:10px}.trend-chip{background:#f5f1f7;border-radius:999px;padding:5px 8px;font-size:11px}.trend-empty{padding:14px;border-radius:15px;background:#faf8fb;color:var(--muted);font-size:13px;line-height:1.45}@media(max-width:850px){.trending-grid{grid-template-columns:1fr}.trending-panel{padding:16px}}`;
