...
function redisConfig() {
  return {
    url:
      process.env.STORAGE_URL || process.env.STORAGE_KV_REST_API_URL || process.env.STORAGE_REST_API_URL || process.env.KV_REST_API_URL || process.env.UPSTASH_REDIS_REST_URL,
    token:
      process.env.STORAGE_TOKEN || process.env.STORAGE_KV_REST_API_TOKEN || process.env.STORAGE_REST_API_TOKEN || process.env.KV_REST_API_TOKEN || process.env.UPSTASH_REDIS_REST_TOKEN
  };
}
...
function baseKey(placeId) { return `vibe-check:active:${String(placeId).slice(0, 500)}`; }
function visitorStateKey(placeId, visitorId) { return `${baseKey(placeId)}:visitor:${String(visitorId).slice(0, 120)}`; }
function activeIndexKey(placeId) { return `${baseKey(placeId)}:index`; }
function activeVibeKey(placeId) { return `${baseKey(placeId)}:vibes`; }
function rateKey(visitorId) { return `vibe-check:rate:checkin:${String(visitorId).slice(0, 120)}`; }
...
function countsFromEntries(entries) {
  const counts = {};
  for (const vibe of entries) if (ALLOWED.includes(vibe)) counts[vibe] = (counts[vibe] || 0) + 1;
  return counts;
}
...
function communitySignal(counts, total, dominant) {
  const weightedTotal = Object.entries(counts).reduce((sum, [vibe, count]) => sum + (COMMUNITY_VIBE_VALUES[vibe] || 0) * Number(count || 0), 0);
  const average = total ? Number((weightedTotal / total).toFixed(1)) : null;
  const confidence = communityConfidence(total);
  const dominantCount = dominant ? Number(counts[dominant] || 0) : 0;
  ...
  return {
    ...
    average,
    dominant,
    dominantPercent: total ? Math.round((dominantCount / total) * 100) : 0,
    confidence,
    influencePercent: Math.round(confidence * 100),
...
async function cleanup(placeId) {
  ...
  if (visitors.length) await redis(["ZREM", indexKey, ...visitors]);
...
async function getLive(placeId) {
  ...
  return { available: true, counts, total, dominant, windowMinutes: Math.round(CHECKIN_WINDOW_SECONDS / 60), community: communitySignal(counts, total, dominant) };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!placeId) return res.status(400).json({ error: "A valid placeId is required." });
    if (req.method === "GET") return res.status(200).json(await getLive(placeId));
    ...
    if (!visitorId) { visitorId = makeVisitorId(); setVisitorCookie(res, visitorId); }
    if (!(await enforceRateLimit(visitorId))) return res.status(429).json({ error: "You've checked in a lot. Please wait a minute before trying again." });

    const { indexKey, vibeKey } = await cleanup(placeId);
    const visitorKey = visitorStateKey(placeId, visitorId);
    const previous = await redis(["HGET", vibeKey, visitorId]);
    const previousVibe = previous?.result && ALLOWED.includes(previous.result) ? previous.result : null;

    if (previousVibe === vibe) {
      const live = await getLive(placeId);
      return res.status(200).json({ ok: true, alreadyCheckedIn: true, placeId, vibe, ...live });
    }

    await redis(["HSET", vibeKey, visitorId, vibe]);
    if (!previousVibe) await redis(["ZADD", indexKey, Math.floor(Date.now() / 1000) + CHECKIN_WINDOW_SECONDS, visitorId]);
    await redis(["SET", visitorKey, vibe, "EX", CHECKIN_WINDOW_SECONDS]);

    const live = await getLive(placeId);
    return res.status(200).json({ ok: true, alreadyCheckedIn: false, updatedVibe: Boolean(previousVibe), placeId, vibe, recordedAt: new Date().toISOString(), ...live });
  } catch (error) {
